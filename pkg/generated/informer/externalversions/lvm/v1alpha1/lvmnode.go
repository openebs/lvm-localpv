...
// LVMNodeInformer provides access to a shared informer and lister for
// LVMNodes.
...
// NewLVMNodeInformer constructs a new informer for LVMNode type.
...
func NewLVMNodeInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredLVMNodeInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredLVMNodeInformer constructs a new informer for LVMNode type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.LocalV1alpha1().LVMNodes(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.LocalV1alpha1().LVMNodes(namespace).Watch(options)
			},
		},
		&lvmv1alpha1.LVMNode{},
		resyncPeriod,
...
func (f *lVMNodeInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredLVMNodeInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
