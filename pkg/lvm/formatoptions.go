...
// formattableFsTypes are the filesystems the driver puts on an lv, the only
// ones a mkfs option can be meant for. A raw block volume is never formatted
...
// agent and keeps them as the format defaults of this node. An entry that can
// not be used is an error, so that a mistyped flag is caught at startup instead
// of silently formatting volumes the node can not mount.
func SetDefaultFormatOptions(entries []string) error {
	...
	for _, entry := range entries {
		...
		// an entry with no options, "xfs=", leaves that filesystem without a
		// default, which is how a value of the chart is turned off
...
// node otherwise. The two are not merged, a storage class value replaces the
// default of its filesystem.
